...
void sandpiles_sum(int grid1[3][3], int grid2[3][3])
{
    ...
    //perform the addition of the grids
    for (row = 0; row < 3; row++)
    {
...
/**
 * topplesand - Topple sands that exist in grid
 * @grid: 3x3 grid
 *
 */
void topple_sand(int grid1[3][3])
{
    ...
    for (row = 0; row < 3; row++)
    {
        for (column = 0; column < 3; column++)
        {
            // up = grid1[row - 1][column];
            // down = grid1[row + 1][column];
            // left = grid1[row][column - 1];
            // right = grid1[row][column + 1];
